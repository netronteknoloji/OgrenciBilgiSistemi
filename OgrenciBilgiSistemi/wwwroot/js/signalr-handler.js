...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("offline", () =>
    showTransientBanner("Bağlantı yok (offline).", "#fde68a"),
  );
  window.addEventListener("online", () =>
    showTransientBanner("Bağlantı geri geldi.", "#bbf7d0"),
  );
});
...
function lunchAllowedFromEnum(o) {
  ...
  if (Object.prototype.hasOwnProperty.call(o, "oglenCikisDurumu")) {
    ...
    if (typeof v === "number")
      return { allowed: v === 0, source: "camel-num", raw: v };
    if (typeof v === "string") {
      const s = v.trim().toLowerCase();
      if (s === "0" || s === "evet")
        return { allowed: true, source: "camel-str", raw: v };
      if (s === "1" || s === "hayır" || s === "hayir")
        return { allowed: false, source: "camel-str", raw: v };
    }
  }
  if (Object.prototype.hasOwnProperty.call(o, "OglenCikisDurumu")) {
    const v = o.OglenCikisDurumu;
    if (typeof v === "number")
      return { allowed: v === 0, source: "pascal-num", raw: v };
    if (typeof v === "string") {
      const s = v.trim().toLowerCase();
      if (s === "0" || s === "evet")
        return { allowed: true, source: "pascal-str", raw: v };
      if (s === "1" || s === "hayır" || s === "hayir")
        return { allowed: false, source: "pascal-str", raw: v };
    }
  }
  return { allowed: false, source: "default", raw: undefined };
}
...
function getReasonSpec(reason, ctx) {
  ...
  switch ((reason || "").toString()) {
    case "YEMEKHANE_OK":
      return {
        fullBg: "#22c55e",
        text: infoMsg || "YEMEKHANE GEÇİŞİ ONAYLANDI.",
        beep: false,
      };
    case "YEMEKHANE_RED":
      return {
        fullBg: "#dc2626",
        text: errMsg || "ÖĞRENCİNİN YEMEKHANE GEÇİŞ İZNİ YOK!",
        beep: true,
      };
    case "YEMEKHANE_LIMIT":
      return {
        fullBg: "#dc2626",
        text: "Bugün için yemekhane geçiş hakkı kullanıldı!",
        beep: true,
      };
    case "ANA_KAPI_OGLE_OK":
      return {
        fullBg: "#22c55e",
        text: infoMsg || "ÖĞLE ÇIKIŞI ONAYLANDI.",
        beep: false,
      };
    case "ANA_KAPI_OGLE_RED":
      return {
        fullBg: "#dc2626",
        text: errMsg || "ÖĞRENCİNİN ÖĞLE ÇIKIŞ İZNİ YOK!",
        beep: true,
      };
    case "ANA_KAPI_OGLE_LIMIT":
      return {
        fullBg: "#dc2626",
        text: "Bugün için öğle çıkış hakkı kullanıldı!",
        beep: true,
      };
    case "ANA_KAPI_OGLE_DONUS_LIMIT":
      return {
        fullBg: "#dc2626",
        text: "Bugün öğle çıkış ve dönüş zaten yapılmış!",
        beep: true,
      };
    default:
      return { fullBg: "#e5e7eb", text: "GEÇİŞ BİLGİSİ", beep: false };
  }
}
...
function renderStudentFragment(o, lunch) {
  ...
  colImg.className =
    "col-md-5 d-flex align-items-center justify-content-center p-3";
  ...
  const isReject =
    ...
    (o.gecisTipi || o.GecisTipi || "").toString().toLowerCase() ===
      "reddedildi";
  ...
  if (isYemekhane) {
    ...
    if (reasonStr === "ANA_KAPI_OGLE_OK") {
      ...
      badge.textContent = lunch.allowed
        ? "Öğle Çıkış: İzin Var"
        : "Öğle Çıkış: İzin Yok";
...
/* =================== Main Flow =================== */
function scheduleEkranTemizle() {
  displayTimeoutId = setTimeout(() => {
    if (ogrenciBilgisiContainer) ogrenciBilgisiContainer.innerHTML = "";
    ...
    console.debug("[kart] ekran temizlendi");
...
function showStudentInfo(o) {
  ...
  if (
    pageGuid &&
    dtoGuid &&
    typeof sameGuid === "function" &&
    !sameGuid(pageGuid, dtoGuid)
  ) {
    // Yabancı cihazın mesajı: kendi ekranımıza dokunma; ama bizim mevcut içerik
    // gösterimde ise onun temizliğini yine garanti altına al.
    if (
      ogrenciBilgisiContainer &&
      ogrenciBilgisiContainer.childElementCount > 0
    ) {
      scheduleEkranTemizle();
    }
    return;
  }

  if (!o || typeof o !== "object") {
    ogrenciBilgisiContainer.replaceChildren(
      renderErrorFragment("Geçersiz veri alındı."),
    );
    scheduleEkranTemizle();
    return;
  }
  ...
  const ANA_KAPI_SET = new Set(["anakapi", "ana kapi", "ana kapı"]);
  if (
    effectiveReason === "GENEL_OK" &&
    ANA_KAPI_SET.has(istasyon) &&
    gecisTipi === "giriş"
  ) {
  ...
  connection
    ...
    .catch((err) => {
      ...
      if (retryCount <= config.maxRetryAttempts) {
        ...
        console.log(
          `Yeniden denenecek (${retryCount}. deneme) ${retryDelay}ms sonra...`,
        );
        setTimeout(startConnection, retryDelay);
      } else {
        console.error(
          `Maksimum yeniden deneme sayısına ulaşıldı (${config.maxRetryAttempts}).`,
        );
